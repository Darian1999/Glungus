...
public final class FireHud {
    private static final Identifier HUD_ID = Identifier.of(Glungus.MOD_ID, "fire_hud");

    private FireHud() {
    }

    public static void initialize() {
        HudElementRegistry.attachElementAfter(VanillaHudElements.BOSS_BAR, HUD_ID, FireHud::render);
    }
    ...
    private static void render(DrawContext drawContext, RenderTickCounter tickCounter) {
        ...
        if (!"fire".equals(slot.power)) {
            return;
        }

        HudData.Layout layout = HudData.layout();
        HudData.PowerConfig config = HudData.power("fire");
        ...
        boolean fireImmuneActive = (slot.flags & PowerStatusPayload.FIRE_IMMUNE_ACTIVE) != 0;
