...
public final class WaterHud {
    private static final Identifier HUD_ID = Identifier.of(Glungus.MOD_ID, "water_hud");

    private WaterHud() {
    }

    public static void initialize() {
        HudElementRegistry.attachElementAfter(VanillaHudElements.BOSS_BAR, HUD_ID, WaterHud::render);
    }
    ...
    private static void render(DrawContext drawContext, RenderTickCounter tickCounter) {
        ...
        if (!"water".equals(slot.power)) {
            return;
        }

        HudData.Layout layout = HudData.layout();
        HudData.PowerConfig config = HudData.power("water");
