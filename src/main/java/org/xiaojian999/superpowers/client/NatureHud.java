...
public final class NatureHud {
    private static final Identifier HUD_ID = Identifier.of(Glungus.MOD_ID, "nature_hud");

    private NatureHud() {
    }

    public static void initialize() {
        HudElementRegistry.attachElementAfter(VanillaHudElements.BOSS_BAR, HUD_ID, NatureHud::render);
    }
    ...
    private static void render(DrawContext drawContext, RenderTickCounter tickCounter) {
        ...
        if (!"nature".equals(slot.power)) {
            return;
        }

        HudData.Layout layout = HudData.layout();
        HudData.PowerConfig config = HudData.power("nature");
        ...
        boolean flowerTrailActive = (slot.flags & PowerStatusPayload.NATURE_FLOWER_TRAIL_ACTIVE) != 0;
