...
public final class IceHud {
    private static final Identifier HUD_ID = Identifier.of(Glungus.MOD_ID, "ice_hud");

    private IceHud() {
    }

    public static void initialize() {
        HudElementRegistry.attachElementAfter(VanillaHudElements.BOSS_BAR, HUD_ID, IceHud::render);
    }
    ...
    private static void render(DrawContext drawContext, RenderTickCounter tickCounter) {
        ...
        if (!"ice".equals(slot.power)) {
            return;
        }

        HudData.Layout layout = HudData.layout();
        HudData.PowerConfig config = HudData.power("ice");
        ...
        boolean snowballPrimed = (slot.flags & PowerStatusPayload.SNOWBALL_PRIMED) != 0;
