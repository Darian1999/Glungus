...
public final class AirHud {
    private static final Identifier HUD_ID = Identifier.of(Glungus.MOD_ID, "air_hud");

    private AirHud() {
    }

    public static void initialize() {
        HudElementRegistry.attachElementAfter(VanillaHudElements.BOSS_BAR, HUD_ID, AirHud::render);
    }
    ...
    private static void render(DrawContext drawContext, RenderTickCounter tickCounter) {
        ...
        if (!"air".equals(slot.power)) {
            return;
        }

        HudData.Layout layout = HudData.layout();
        HudData.PowerConfig config = HudData.power("air");
        ...
        boolean airFlightActive = (slot.flags & PowerStatusPayload.AIR_FLIGHT_ACTIVE) != 0;
